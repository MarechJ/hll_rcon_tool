...
class AutoModNoLeaderUserConfig(BaseUserConfig):
    ...
    enabled: bool = Field(
        default=False,
        title="Enable",
        description="Whether the Automod is enabled or not",
    )
    dry_run: bool = Field(
        default=False,
        title="Dry-Run",
    ...
    number_of_notes: int = Field(
        ge=0,
        default=1,
        title="Number of notes",
        description="The amount of 'notes' the Automod does before transitioning to the next punishment level. Notes do not have any visible action to the player",
    )
    notes_interval_seconds: int = Field(
        ge=1,
        default=10,
        title="Notes interval",
        description="The time in seconds between the same player is 'noted' by the Automod",
    )
