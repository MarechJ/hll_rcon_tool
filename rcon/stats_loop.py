...
class Series:
    ...
    def __init__(self, client=None, min_resolution_ms=30000, retention_days=60, max_fails=5, slices=None):
    ...
    def migrate(self):
        try:
            logger.debug("Creating timeseries %s with ms retention %s", 
                        self.NAME, self.retention_msecs)
            self.client.create(self.NAME, retention_msecs=self.retention_msecs)
        except redis.exceptions.ResponseError:
            logger.debug("Already exists. Updating timeseries %s with ms retention %s", 
                        self.NAME, self.retention_msecs)
        ...
        for rule_name, agg_method, time_resolution in self.slices.values():
            ...
            try:
                ...
                self.client.createrule(self.NAME, rule_name, agg_method, time_resolution)
            except redis.exceptions.ResponseError:
