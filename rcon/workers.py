...
def _record_stats(map_info=None, map=None):
    ...
    with enter_session() as sess:
        map_ = get_or_create_map(sess=sess, start=start, end=end, server_number=os.getenv("SERVER_NUMBER"), map_name=map_info["name"])
...
def record_stats_from_map(sess, map_):
    ...
    player_stats = stats.get_players_stats_at_time(from_=map_.start, until=map_.end, server_number=str(map_.server_number))
