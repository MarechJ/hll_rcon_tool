...
def save_missing_match_logs_worker(map_info: MapInfo) -> Job:
    ...
    return queue.enqueue(
        save_missing_match_logs, map_info, retry=Retry(max=15, interval=30)
    )
...
def save_missing_match_logs(map_: MapInfo):
    ...
    try:
        ...
        with enter_session() as sess:
            if not _are_match_logs_available(
                sess, int(get_server_number()), match_start, match_end
            ):
                raise ValueError(
                    "match logs are not yet available, skipping backtracking logs"
                )
        ...
        rcon_match_logs = [
            log
            for log in rcon_logs["logs"]
            if match_start <= log["event_time"].replace(tzinfo=datetime.UTC)
            and log["event_time"].replace(tzinfo=datetime.UTC) <= match_end
        ]
        # 20_000 logs should be enough (90 min warfare full server results in approx 4000 logs)
        match_redis_logs = [
            log
            for log in LogsHistory()[:20000]
            if match_start <= log["event_time"].replace(tzinfo=datetime.UTC)
            and log["event_time"].replace(tzinfo=datetime.UTC) <= match_end
        ]
        ...
        if len(rcon_match_logs) == 0:
            logger.info(
                "This match is probably too old and the HLLSERVER does not retain those logs any longer"
            )
            logger.info("[WORKER] - Done backtracking logs")
            return

        logger.info(
            "First log: %s, Last log: %s",
            rcon_match_logs[-1]["raw"],
...
def record_stats_worker(map_info: MapInfo, depends_on: Job | None = None):
    ...
    if depends_on:
        ...
        queue.enqueue(
            record_stats,
            map_info,
            retry=Retry(max=15, interval=30),
...
def record_stats(map_info: MapInfo):
    logger.info(
        "Recording stats for %s", (map_info["name"], map_info["start"], map_info["end"])
    )
    try:
        _record_stats(map_info)
        logger.info(
            "Done recording stats for %s",
            (map_info["name"], map_info["start"], map_info["end"]),
        )
    except Exception:
...
def clear_stats_cache(map: Maps, map_info: MapInfo | None):
    ...
    map_index, map_to_update = next(
        (
            (idx, m)
            for idx, m in enumerate(maps_history)
            if m["start"] == map_info["start"]
            and m["end"] == map_info["end"]
            and m["name"] == map_info["name"]
        ),
        (None, None),
    )
...
def _get_game_logs_stats(
    session: Session, map: Maps, cached_players: dict[str, PlayerStat] | None = None
):
    game_log_stats = TimeWindowStats()
    return game_log_stats.get_players_stats_at_time(
        from_=map.start.replace(tzinfo=UTC),
        until=map.end.replace(tzinfo=UTC),
        server_number=str(map.server_number),
        cached_players=cached_players or {},
    )
...
def record_stats_from_map(
    ...
    for player_game_log_stats in _get_game_logs_stats(sess, map_, temp_stats).values():
        ...
        if not player_id:
            logger.error(
                "Stat object does not contain a player ID: %s", player_game_log_stats
            )
            continue
