import { IconButton, InputLabel, Paper, Tooltip, Typography } from '@mui/material'
...
const BlacklistListTile = ({ blacklist, servers, onEdit, onDelete }) => {
  ...
  return (
    <Paper>
      <Grid container justifyContent='space-between' alignItems='center' spacing={2} style={{ paddingLeft: 18 }}>
        <Grid>
        ...
        <Grid size={2}>
          ...
          <Typography align='left'>{blacklist.name}</Typography>
        </Grid>
        ...
        <Grid>
          ...
          <Tooltip title='Delete'>
            <span>
              <IconButton onClick={() => onDelete(blacklist)} disabled={blacklist.id === 0} size='large'>
                <DeleteIcon />
              </IconButton>
            </span>
          </Tooltip>
