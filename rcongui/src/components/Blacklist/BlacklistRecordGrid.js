...
const BlacklistRecordTile = ({ record, onEdit, onExpire, onDelete }) => {
    ...
    >
      <MaterialList>
        <ListItem component={'div'} alignItems='flex-start' style={{ paddingBottom: 0 }}>
          <ListItemAvatar>
            <Link target='_blank' color='inherit' href={makePlayerProfileUrl(player.get('player_id'), firstName)}>
              ...
              <Fragment>
                {showAll ? (
                  <Typography variant='body1'>
                    {hasMultipleNames ? (
                      <IconButton size='small' color='primary' onClick={() => setShowAll(false)}>
                  ...
                  <Typography variant='body1'>
                    {hasMultipleNames ? (
                      <IconButton size='small' color='primary' onClick={() => setShowAll(true)}>
      ...
      <Typography variant={reason.length > 65 ? 'body2' : 'body1'}>
        {reason.length > 110 ? reason.substring(0, 108) + '...' : reason}
      </Typography>
...
const BlacklistRecordGrid = ({ blacklists, records, onRefresh }) => {
  ...
  function onEditDialogSubmit(data) {
    ...
    postData(`${process.env.REACT_APP_API_URL}edit_blacklist_record`, {
      ...
      .then((response) => showResponse(response, `Record ${recordId} was edited`, true))
  ...
  function onExpireRecord(record) {
    postData(`${process.env.REACT_APP_API_URL}edit_blacklist_record`, {
      ...
      .then((response) => showResponse(response, `Record ${record.get('id')} was edited`, true))
      .then((result) => result && !result.failed && onRefresh())
      .catch(handle_http_errors)
  }
  ...
  function onDeleteRecord(record) {
    postData(`${process.env.REACT_APP_API_URL}delete_blacklist_record`, {
      ...
      .then((response) => showResponse(response, `Record ${record.get('id')} was deleted`, true))
      .then((result) => result && !result.failed && onRefresh())
      .catch(handle_http_errors)
  }
      ...
      <Grid container>
        <Grid size={12}>
          <ImageList cols={size} cellHeight={210} spacing={12}>
            {records.map((record) => {
              return (
                <ImageListItem key={record.get('id')}>
                  <BlacklistRecordTile
                    record={record}
                    onEdit={onEditRecord}
