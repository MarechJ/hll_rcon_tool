...
const DesktopHeader = ({ data }) => {
  return (
    <DesktopHeaderWrapper container component={'header'}>
      ...
      <Grid size={4}>{data ? <Board data={data} /> : <StyledSkeleton />}</Grid>
      <Grid
        size={4}
        sx={{
          pl: 1,
...
const MobileHeader = ({ data }) => {
  return (
    <MobileHeaderWrapper container component={'header'}>
      <Grid size={12}>{data ? <Board data={data} /> : <StyledSkeleton />}</Grid>
      <Grid size={6} sx={{ borderRight: '1px solid', borderRightColor: (theme) => theme.palette.divider }}>
        {data ? <TeamMobile data={data.allies} align={'start'} /> : <StyledSkeleton />}
      </Grid>
      <Grid size={6}>{data ? <TeamMobile data={data.axis} align={'end'} /> : <StyledSkeleton />}</Grid>
    </MobileHeaderWrapper>
  )
}
