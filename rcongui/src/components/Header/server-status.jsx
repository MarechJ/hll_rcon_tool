...
const ServerStatus = () => {
  ...
  const balance = `${status?.player_count_by_team?.allied ?? 0}vs${status?.player_count_by_team?.axis ?? 0}`
  ...
  return (
    <Wrapper>
      ...
      <Typography variant='caption'>
        {numCurrentPlayers}/{maxPlayers} ({balance}) - {mapName} - {timeRemaining} - {score}
      </Typography>
    </Wrapper>
  )
}
