    ...
    <StyledCard>
      ...
      >
        {sortedLogs.map((group, index) => (
          <Tab key={group.group} label={`${logActions[group.group] || ''} ${group.group} (${group.logs.length})`} />
        ))}
      </Tabs>
      <ScrollableContent>
        <List disablePadding>
            ...
            const Message = () => {
              ...
              message.forEach((part, index) => {
                if (index % 2 === 0) {
                  ...
                  if (log.user_1?.id) {
                    output.push(
                      <PlayerLink playerId={log.user_1?.id} key={index + 'b' + 'user_1'}>
                        {log.user_1?.name}
                      </PlayerLink>
                    )
                  } else {
              ...
              if (message[message.length - 1] && message[message.length - 1] !== '') {
                ...
                messageEnd.forEach((part, index) => {
                  if (index % 2 === 0) {
                    ...
                    if (log.user_2?.id) {
                      output.push(
                        <PlayerLink playerId={log.user_2?.id} key={index + 'b' + log.user_2?.id}>
                          {log.user_2?.name}
                        </PlayerLink>
                      )
                    } else {
