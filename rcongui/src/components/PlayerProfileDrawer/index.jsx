...
const BasicProfileDetails = ({ firstSeen, lastSeen, country, vip, sessionCount, flags, totalPlaytime }) => {
  return (
    <dl>
      <dt>Country</dt>
      <dd>
  ...
  return (
    <>
      ...
      <Accordion defaultExpanded>
        <AccordionSummary expandIcon={<ExpandMoreIcon />} aria-controls='panel-action-content' id='panel-action-header'>
          Show all
        </AccordionSummary>
        <AccordionDetails>{`This player received ${actions.length} actions in total.`}</AccordionDetails>
        <AccordionActions>
          <Button onClick={() => setExpanded((prevState) => !prevState)}>{expanded ? 'Hide' : 'Show'}</Button>
        </AccordionActions>
      </Accordion>
    </>
  )
  ...
  return (
    <ProfileWrapper component={'article'}>
      <ProfileHeader rowGap={1}>
        ...
        <Box sx={{ flexGrow: 1 }}>
          <Typography component={'h1'} variant='h6' sx={{ textOverflow: 'ellipsis' }}>
            {name}
          </Typography>
      ...
      <TabContext value={openedTab}>
        <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
          ...
          >
            ...
            <Tab
              label='Messages'
              value='messages'
              disabled={profile.received_actions.filter((action) => action.action_type === 'MESSAGE').length === 0}
            />
            <Tab label='Comments' value='comments' disabled={player.comments.length === 0} />
            <Tab label='Bans' value='bans' disabled={player.bans.length === 0} />
        ...
        <TabPanel value='profile'>
          <BasicProfileDetails
            ...
            totalPlaytime={profile.total_playtime_seconds ?? profile.total_playtime_seconds}
            vip={playerVip}
          />
        </TabPanel>
        {isOnline && (
        ...
        <TabPanel value='messages'>
          <Box component={'section'}>
            ...
            <Messages messages={profile.received_actions.filter((action) => action.action_type === 'MESSAGE')} />
          </Box>
        </TabPanel>
...
export const PlayerDetailDrawer = () => {
  ...
  return (
    <ResponsiveDrawer variant='persistent' open={open} anchor='right' onClose={close}>
      <Toolbar />
      {isLoading && !player ? (
