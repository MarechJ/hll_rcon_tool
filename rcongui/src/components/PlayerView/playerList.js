...
import { Avatar, Badge, Checkbox, Chip, ListItemAvatar } from '@mui/material'
...
const getBans = (profile) => {
  return profile.get('steam_bans') && profile.get('steam_bans').get('has_bans') === true ? (
    <WithPopOver content={`Players has bans: ${JSON.stringify(profile.get('steam_bans'))}`}>
...
const Flag = ({ data, onDeleteFlag }) => {
  return (
    <Link onClick={() => (window.confirm('Delete flag?') ? onDeleteFlag(data.get('id')) : '')}>
      <WithPopOver content={`Comment: ${data.get('comment')}`}>{getEmojiFlag(data.get('flag'), 22)}</WithPopOver>
    </Link>
  )
}
...
const formatPunitions = (profile) => {
  ...
  return profile
    ...
    .map((item) => (
      <Typography display='block' variant='caption' key={item}>{`${formatTime(item)} ${item.get(
        'action_type'
      )} by ${item.get('by')}: ${item.get('reason')}`}</Typography>
    ))
}
    ...
    <Fragment>
      <Grid>
        <Chip
          size='small'
          variant='outlined'
          avatar={<Avatar alt='Combat' src='icons/roles/score_combat.png' />}
      ...
      <Grid>
        <Chip
          size='small'
          variant='outlined'
          avatar={<Avatar alt='Offensive' src='icons/roles/score_offensive.png' />}
      ...
      <Grid>
        <Chip
          size='small'
          variant='outlined'
          avatar={<Avatar alt='Defensive' src='icons/roles/score_defensive.png' />}
      ...
      <Grid>
        <Chip
          size='small'
          variant='outlined'
          avatar={<Avatar alt='Support' src='icons/roles/score_support.png' />}
    ...
    <Fragment>
      ...
      <Grid>
        <Chip
          size='small'
          variant='outlined'
          label={`KD: ${(player.get('kills', 0) / Math.max(player.get('deaths', 1), 1)).toFixed(2)}`}
        />
      </Grid>
      {player.get('loadout') ? (
        <Grid>
    ...
    <ListItem key={name} dense selected={isSelected}>
      {playerHasExtraInfo ? (
        <ListItemAvatar>
          ...
          >
            <Avatar variant='square' src={`icons/roles/${player.get('role', 'rifleman')}.png`}></Avatar>
          </Badge>
        </ListItemAvatar>
      ) : (
        ''
      ...
      <ListItemText
        ...
        primary={
          <Fragment>
            ...
            <Link target='_blank' color='inherit' href={makePlayerProfileUrl(player_id, name)}>
              <FontAwesomeIcon icon={player_id.length === 17 ? faSteam : faWindows} />
            </Link>
          </Fragment>
        }
        secondary={
          <Fragment>
            <span>
              {seconds_to_time(profile.get('current_playtime_seconds'))} - #{profile.get('sessions_count')} -{' '}
              {getBans(player)}
            </span>{' '}
            <Link color='inherit' component={RouterLink} to={`/records/players/${player.get('player_id')}`}>
