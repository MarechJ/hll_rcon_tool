  ...
  return (
    <ListItem key={groupName} disablePadding sx={{ display: 'block' }}>
      ...
      <Collapse in={open} timeout={'auto'} unmountOnExit>
        <List dense sx={{ '& .MuiListItemButton-root': { pl: 2 * level } }} disablePadding>
          {children}
        </List>
      </Collapse>
    </ListItem>
    ...
    >
      <List dense sx={{ overflowY: 'auto' }}>
        {navigationTree
          ...
          .map((group) =>
            group.links.map((link) => <NavigationLink key={link.to} to={link.to} text={link.name} icon={link.icon} />)
          )}
