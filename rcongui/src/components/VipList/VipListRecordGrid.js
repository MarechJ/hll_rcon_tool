...
const VipListRecordTile = ({
  ...
  const firstName = playerNames.get(0)
    ? playerNames.get(0).get("name")
    : undefined;
  ...
  return (
    ...
    >
        ...
        >
          <ListItemAvatar>
            <Link
              target="_blank"
              color="inherit"
              href={makePlayerProfileUrl(player.get("player_id"), firstName)}
            >
      ...
      <Typography variant={notes?.length > 65 ? "body2" : "body1"}>
        {notes?.length > 110 ? notes?.substring(0, 108) + "..." : notes}
      </Typography>
      ...
      <Grid container justifyContent="space-between" spacing={0}>
        <Grid container justifyContent="space-around" spacing={0}>
          <Grid>
            <Tooltip
              title={expiresAt ? expiresAt.format("LLLL") : "Permanent"}
              arrow
            >
              <small>
                {expiresAt
...
const VipListRecordGrid = ({ vipLists, records, onRefresh }) => {
  ...
  function onEditDialogSubmit(data) {
    ...
    if (Object.keys(payload).length > 0) {
      postData(`${process.env.REACT_APP_API_URL}update_player_profile`, {
  ...
  function onInactivateRecord(record) {
    postData(`${process.env.REACT_APP_API_URL}edit_vip_list_record`, {
      ...
      .then((response) =>
        showResponse(
          response,
          `Record ${record.get("id")} was deactivated`,
          true
        )
      )
      .then((result) => result && !result.failed && onRefresh())
