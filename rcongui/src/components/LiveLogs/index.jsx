...
const limitOptions = [100, 250, 500, 1000, 2500, 5000, 10000, 25000, 50000, 100000]
...
const LiveLogs = ({ initialLogsView }) => {
  ...
  const { data: logsView, error } = useQuery({
    queryKey: ['logs', 'live'],
    queryFn: cmd.GET_RECENT_LOGS({
      params: {
    ...
    <Stack>
      ...
      >
        ...
        <Autocomplete
          ...
          renderInput={(params) => (
            <TextField {...params} variant='outlined' label='Filter by player' sx={{ width: 200 }} />
          )}
          renderOption={(props, option, { selected }) => {
            const { key, ...optionProps } = props
            return (
      ...
      <Suspense fallback={<Skeleton variant='rectangular' sx={{ height: '100vh' }} />}>
        {logsConfig.logsMode === 'table' ? (
          <LogTable data={logsView.logs} columns={logColumns} />
        ) : (
          <Paper sx={{ p: 1, my: 1, overflow: 'auto' }} className={logsConfig.highlighted && 'highlighted'}>
            <LogLine
              sx={{
                display: 'inline-flex',
