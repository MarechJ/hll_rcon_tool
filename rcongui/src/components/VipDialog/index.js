 ...
 */
export function VipExpirationDialog({
  open,
  vips,
  onDeleteVip,
  handleClose,
  ...
  useEffect(() => {
    ...
    if (open === true && player) {
      ...
      if (player?.has("is_vip") && player.get("is_vip")) {
        ...
        let p = vips.find(
          (vipObj) => vipObj.player_id === player.get("player_id")
        );
        ...
        setExpirationTimestamp(
          dayjs(pExpiration ? pExpiration : dayjs()).format(
      ...
      if (
        vips.some(
          (playerVIP) => playerVIP.player_id === player.get("player_id")
        )
      ) {
        setIsVip(true);
        setExpirationTimestamp(
          dayjs(player.get("vip_expiration")).format("YYYY-MM-DD HH:mm:ssZ")
        );
        return;
      }
    }
  ...
  const currentVipExpiration =
    typeof open === "object"
      ? open?.get("vip_expiration")
      : player?.has("vip_expiration")
      ? player?.get("vip_expiration")
      : vips?.find((vipObj) => vipObj.player_id === player?.get("player_id"))
          ?.vip_expiration;
  ...
  return (
    ...
    >
      <DialogTitle id="form-dialog-title">Manage VIP</DialogTitle>
      <DialogContent>
        <Box style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          <PlayerVipSummary
            player={player ?? open}
            vipExpiration={currentVipExpiration}
            isVip={isVip}
          />
          ...
          <Box>
            ...
            <Button
              ...
              onClick={() =>
                setExpirationTimestamp("3000-01-01T00:00:00+00:00")
              }
            >
              Indefinite
            </Button>
          ...
          <ForwardCheckBox
            label={
              <span>
                Apply VIP to <strong>ALL</strong> servers?
              </span>
            }
            bool={forward}
