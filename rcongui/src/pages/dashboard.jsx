...
import { List, Card, ListItem, CardHeader, CardContent, styled, Box, Typography, Divider } from '@mui/material'
...
const GameBalanceCard = ({ games }) => {
  const times = games.map((game) => new Date(game.start)).reverse()
  const scores = games.map((game) => game.result.allied - game.result.axis).reverse()
      ...
      <CardContent>
        <LineChart
          ...
          series={[
            {
              data: scores,
              valueFormatter: (value) => (value === 0 ? 'Unfinished game' : value > 0 ? 'Allies win' : 'Axis win')
            }
          ]}
          height={CHART_HEIGHT}
        />
          ...
          {games.map((game) => {
            ...
            return (
              ...
              >
                ...
                <Divider orientation='vertical' flexItem sx={{ mx: 2 }} />
                <Typography variant='subtitle2'>{dayjs(game.start).format('HH:mm:ss, MMM DD')}</Typography>
              </ListItem>
            )
          })}
        </List>
...
const Dashboard = () => {
  ...
  const ingameMods = useMemo(
    () =>
      onlineIngameMods.map((mod) => ({
        id: mod.id,
        name: mod.username,
        avatar: onlinePlayers.find((player) => player.player_id === mod.player_id)?.profile?.steaminfo?.profile?.avatar
      })),
    [onlineIngameMods]
  )
  ...
  const { data: gamesList = [], isLoading: isLoadingGames } = useQuery({
    ...
    queryFn: async () => {
      ...
      return result.maps.filter((game) => game.server_number === status.server_number).slice(0, 15)
    }
  })
  ...
  const detailedGames = useMemo(() => {
    return gameQueries.filter((query) => query.isSuccess).map((query) => query.data)
  }, [gameQueries])
  ...
  const { data: logs = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const adminLogs = rawLogs.result.logs
        .filter((log) => log.action.startsWith('CHAT') && log.message.includes('!admin'))
        .map((log) => ({
          user_1: {
            id: log.player_id_1,
            name: log.player_name_1
      ...
      <Grid size={SMALL_CARD_SIZE}>
        <ScrollableCard title='Map Rotation'>
          <Box component='ol' sx={{ listStylePosition: 'inside', pl: 0 }}>
            {mapRotation.map((map) => (
              <Box component='li' key={map.id} sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.25 }}>
