...
const ConsoleAdminsPage = () => {
  ...
  const filteredAdmins = useMemo(
    () => admins.filter((admin) => admin.name.toLowerCase().includes(searched.toLowerCase())),
    [admins, searched]
  )
  ...
  const handleFileDownload = () => {
    const rows = admins.map((admin) => [`${admin.player_id}\t${admin.name}\t${admin.role}`])
  ...
  const isValidAdminInput = useMemo(() => Object.entries(newAdmin).every(([_, value]) => !!value.trim()), [newAdmin])
  ...
  const hasChanges = useMemo(() => {
    ...
    const uniqueIds = new Set(
      serverAdmins.map((admin) => admin.player_id).concat(admins.map((admin) => admin.player_id))
    )
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Button onClick={submitChanges} disabled={!hasChanges} variant='contained'>
          Apply
        </Button>
      </Stack>
      <Stack
      ...
      >
        <Stack direction={'row'} gap={1} sx={{ mb: 1, p: 0.5 }}>
          ...
          <Button onClick={handleAddItem} disabled={!isValidAdminInput} variant='contained'>
            Add
          </Button>
        </Stack>
        <Stack direction={'row'} alignItems={'center'} justifyContent={'space-between'} gap={1} sx={{ mb: 1, p: 0.5 }}>
          <FormControlLabel
            ...
            control={
              <Checkbox
                ...
                checked={checked.size === filteredAdmins.length && checked.size !== 0}
                indeterminate={checked.size > 0 && checked.size !== filteredAdmins.length}
                size='small'
          ...
          <SearchInput onChange={onSearchedChange} placeholder='Search console admin' />
          <Divider orientation='vertical' flexItem />
          <Button
              ...
              <ListItem
                ...
                secondaryAction={
                  <Stack direction={'row'} gap={1}>
                    <IconButton onClick={() => handleOpenProfile(admin.player_id)}>
                      <AccountCircleIcon />
                    </IconButton>
                    <IconButton onClick={() => handleDeleteSingleItem(admin.player_id)}>
                      <DeleteIcon />
                    </IconButton>
                  </Stack>
                }
              >
                <ListItemButton role={undefined} onClick={handleToggle(admin.player_id)} dense>
                  <ListItemIcon>
                    <Checkbox
                      edge='start'
                  ...
                  <ListItemText id={labelId} primary={admin.name} secondary={admin.role} />
                </ListItemButton>
              </ListItem>
            )
          })}
