...
const VipPage = () => {
  ...
  const filteredItems = useMemo(
    () => clientItems.filter((item) => item.name.toLowerCase().includes(searched.toLowerCase())),
  ...
  const handleFileDownload = () => {
    const rows = clientItems.map((vip) => [`${vip.player_id}\t${vip.name}\t${vip.vip_expiration}`])
  ...
  const isValidAddFormInput = useMemo(
    () => Object.entries(addFormData).every(([_, value]) => value ?? !!value.trim()),
    [addFormData]
  )
  ...
  const hasChanges = useMemo(() => {
    ...
    const uniqueIds = new Set(serverItems.map((vip) => vip.player_id).concat(clientItems.map((vip) => vip.player_id)))
    ...
    >
      ...
      >
        <Stack direction={'row'} gap={1} sx={{ mb: 1, p: 0.5 }}>
          ...
          <Button onClick={handleAddItem} disabled={!isValidAddFormInput} variant='contained'>
            Add
          </Button>
        </Stack>
          ...
          <FormControlLabel
            ...
            control={
              <Checkbox
                ...
                checked={checked.size === filteredItems.length && checked.size !== 0}
                indeterminate={checked.size > 0 && checked.size !== filteredItems.length}
                size='small'
              ...
              <ListItem
                ...
                secondaryAction={
                  <Stack direction={'row'} gap={1}>
                    <IconButton onClick={() => handleOpenProfile(vip.player_id)}>
                      <AccountCircleIcon />
                    </IconButton>
                    <IconButton onClick={() => handleDeleteSingleItem(vip.player_id)}>
                      <DeleteIcon />
                    </IconButton>
                  </Stack>
                }
              >
                <ListItemButton role={undefined} onClick={handleToggle(vip.player_id)} dense>
                  <ListItemIcon>
                    <Checkbox
                      edge='start'
