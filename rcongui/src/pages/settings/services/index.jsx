...
const Services = () => {
  ...
  return (
    <Stack sx={{ maxWidth: (theme) => theme.breakpoints.values.md }}>
      ...
      >
        Each service generates logs. If you need to report an issue, please include the relevant log report.
      </Alert>
          ...
          <Accordion key={service.name}>
            ...
            >
              <Stack direction={'row'} gap={1} sx={{ minWidth: 300 }}>
                <Chip sx={{ flexBasis: 80 }} color={stateColor[service.statename]} label={service.statename} />
            ...
            <AccordionDetails>
              <Stack component={'section'} gap={4}>
                ...
                <Stack direction={'row'} alignItems={'center'} flexWrap={true} gap={1}>
                  <Typography variant='caption'>
                    {`${isRunning(service) ? 'Started' : 'Stopped'} ${service.uptime}`}
                  </Typography>
                  <Divider orientation='vertical' flexItem />
                  <Typography variant='caption'>Stopped @ {service.stop}</Typography>
                  <Divider orientation='vertical' flexItem />
                  <Typography variant='caption'>Started @ {service.start}</Typography>
                </Stack>
              </Stack>
              <Divider sx={{ my: 0.5 }} orientation='horizontal' flexItem />
              <Typography>{service.info || 'Service description missing.'}</Typography>
            </AccordionDetails>

            <AccordionActions>
              <Form method='POST'>
                <input name={'service_name'} value={service.name} hidden readOnly />
                <Button
                  type='submit'
                  variant='contained'
