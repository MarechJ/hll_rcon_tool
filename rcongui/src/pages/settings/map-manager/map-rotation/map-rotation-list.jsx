...
function Sortable({ thisList, mapLayer, index, id, onRemove, onChange, isSaved }) {
  const { attributes, listeners, setNodeRef, isDragging, transform, transition } = useSortable({ id: id })
...
const Item = ({ thisList, isDragging, mapLayer, index, onRemove, onChange, isSaved }) => {
  ...
  return (
    <ListItem
      ...
      secondaryAction={
        <Stack direction='row' gap={2}>
          ...
          <IconButton edge='end' aria-label='delete' onClick={() => onRemove(index)} size='large'>
            <DeleteIcon />
          </IconButton>
        </Stack>
      }
    >
      ...
      <ListItemText primary={mapLayer.map.pretty_name} secondary={<MapDescription mapLayer={mapLayer} />} />
    </ListItem>
  )
}
  ...
  return (
    ...
    >
      <SortableContext items={items} strategy={verticalListSortingStrategy}>
        {items.map((mapLayer, index, thisList) => (
          <Sortable
            id={mapLayer.id}
            key={mapLayer.id}
            index={index}
