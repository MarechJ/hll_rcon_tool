...
import { generateObjectivesGrid, getTacMapImageSrc, unifiedGamemodeName } from '@/components/MapManager/helpers'
...
const ControlButton = styled('button')(({ theme, state }) => ({
  ...
  '&:disabled': {
    backgroundImage: 'repeating-linear-gradient(45deg, #ff7700 0, #ff7700 2px, transparent 0, transparent 50%)',
...
function MapObjectives() {
  ...
  const handleSelectClick = (index) => {
    ...
    setObjectives((prevObjectives) =>
      prevObjectives.map((row, rowIndex) =>
        row.map((item, itemIndex) => (rowIndex === targetRow && itemIndex === targetIndex ? !item : item))
      )
    )
  }
  ...
  const handleChangeLayoutClick = async () => {
    setIsSaving(true)
    const chosenObjectives = reduceToInts(objectives[1][0] !== null ? flip(objectives) : objectives)
  ...
  useEffect(() => {
    updateServerStatus()
    statusIntervalRef.current = setInterval(updateServerStatus, UPDATE_INTERVAL)
  ...
  return (
    <Container>
      <ActionPanel>
        ...
        <Button variant='outlined' size='small' onClick={handleResetLayoutClick}>
          CLEAR LAYOUT
        </Button>
      </ActionPanel>
      {currentMap ? (
        <ObjectivesContainer>
          <MapWrapper>
            <MapImg src={getTacMapImageSrc(currentMap)} alt='' draggable={false} />
          </MapWrapper>
          <ObjectivesGrid>
