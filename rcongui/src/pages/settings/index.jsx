...
const GeneralSettingsContent = ({ settings }) => {
  const { pendingSettings, setPendingSettings, isSubmitting, error, submit, reset, isAltered } =
    useSettingsState(settings)

  const { handleInputChange, handleBlur, handleSliderChange, handleToggleChange } = useInputHandlers(
    pendingSettings,
    setPendingSettings
  )
  ...
  const submitChanges = (intent) => () => {
    ...
    payload.settings = Object.fromEntries(
      Object.entries(pendingSettings).filter(([key, value]) => JSON.stringify(settings[key]) !== JSON.stringify(value))
    )
  ...
  const addThreshold = () => {
    const startingValue =
      pendingSettings.votekick_thresholds.length > 0
        ? pendingSettings.votekick_thresholds[pendingSettings.votekick_thresholds.length - 1][0] + 1
        : 0
  ...
  const removeThreshold = (index) => {
    const updatedThresholds = pendingSettings.votekick_thresholds.filter((_, i) => i !== index)
  ...
  return (
    <Box>
      <Stack direction={'row'} sx={{ p: 1, mb: 1 }} justifyContent={'end'} alignItems={'center'} gap={1}>
      ...
      <Stack gap={1}>
        <Box>
          <Typography variant='h6' id='autobalance-slider' gutterBottom>
            Autobalance {pendingSettings.autobalance_enabled ? 'Enabled' : 'Disabled'}
          </Typography>
        ...
        <Box sx={{ width: '100%' }}>
          ...
          {pendingSettings.votekick_thresholds.map((threshold, index) => (
            <Grid container spacing={2} key={`threshold-${index}`} sx={{ mb: 2, alignItems: 'center' }}>
              <Grid size={8}>
                <FormControl fullWidth size='small'>
                  <InputLabel htmlFor={`threshold-${index}-min`}>Min. players</InputLabel>
                  <Input
                    ...
                    type='number'
                    inputProps={{
                      min: index > 0 ? pendingSettings.votekick_thresholds[index - 1][0] + 1 : 0,
              ...
              <Grid size={2}>
                <FormControl fullWidth size='small'>
                  <InputLabel htmlFor={`threshold-${index}-votes`}>Votes</InputLabel>
                  <Input
              ...
              <Grid size={2}>
                <Button size='small' onClick={() => removeThreshold(index)} sx={{ minWidth: 'auto' }}>
                  <DeleteIcon />
                </Button>
              </Grid>
            </Grid>
          ...
          {(!pendingSettings.votekick_thresholds.length ||
            pendingSettings.votekick_thresholds[pendingSettings.votekick_thresholds.length - 1][0] < 50) && (
            <Button startIcon={<AddIcon />} onClick={addThreshold} variant='outlined' size='small'>
              Add Threshold
            </Button>
          )}
          <Typography component={'p'} variant='caption' color='text.secondary'>
            Set the number of votes required to kick a player based on the number of players in the team. The first
            field defines the minimum number of players in the team, and the second field defines the number of votes
            required. Reasonable values are [0, 1], [10, 5], [25, 12], [50, 20].
          </Typography>
        </Box>
      </Stack>
    </Box>
...
const AutoVotekickContent = ({ settings }) => {
  ...
  const { handleInputChange, handleBlur, handleToggleChange } = useInputHandlers(pendingSettings, setPendingSettings)
  ...
  return (
    <Box>
      <Stack direction={'row'} sx={{ p: 1, mb: 1 }} justifyContent={'end'} alignItems={'center'} gap={1}>
        ...
        <Button disabled={!isAltered} variant='contained' onClick={submitChanges}>
      ...
      <Stack gap={1}>
        <Typography>
          When enabled, this feature manages the in-game votekicks by turning them off if the specified conditions are
          met, and turning them back on if they are not met. Ingame moderators are those who are actually playing the
          game on the server. Online moderators are those who are logged in CRCON's interface.
        </Typography>
        <Divider flexItem />
        <FormControl required error={pendingSettings.minimum_ingame_mods === ''} sx={{ mt: 2 }}>
          <InputLabel htmlFor={`in-game-moderators`}>Minimum in-game moderators</InputLabel>
          <Input
        ...
        <FormControl required error={pendingSettings.minimum_online_mods === ''} sx={{ mt: 2 }}>
          <InputLabel htmlFor={`online-crcon-moderators`}>Online CRCON moderators</InputLabel>
          <Input
...
const CameraNotificationsContent = ({ settings }) => {
  ...
  const { handleToggleChange } = useInputHandlers(pendingSettings, setPendingSettings)
  ...
  return (
    <Box>
      ...
      <Stack gap={1}>
        <Typography>
          This feature will notify players whenever an admin enters Admin Camera mode (available only on PC servers as
          of version 15.2 of the game). You can choose to send a broadcast message or change the Welcome message.
          However, it is not recommended to change the Welcome message, as doing so will display the camera message
          first, followed by the default text a few seconds later. This will obscure the left side of all players'
          screens twice with each camera entrance.
        </Typography>
        <Divider flexItem />
        <Padlock
  ...
  return (
    <Box>
      ...
      <Stack gap={1}>
        ...
        <Typography>
          When enabled, each VIP that enters the server takes one VIP slot that is not released until the VIP leaves.
          This is achieved by dynamically setting the number of VIP slots based on the formula: (Max number of VIP slots
          - current number of VIPs in-game). The number of VIP slots will never fall below the "Min number of VIP
          slots". You can set this minimum to 0 to create a hard cap. If you use the autoSettings, ensure that you do
          not set the VIPs in there, as it will override the realVIP system.
        </Typography>
        <Divider flexItem />
        <FormControl required error={pendingSettings.minimum_number_vip_slots === ''} sx={{ mt: 2 }}>
          <InputLabel htmlFor={`minimum-vip-slots`}>Minimum VIP slots</InputLabel>
          <Input
        ...
        <FormControl required error={pendingSettings.desired_total_number_vips === ''} sx={{ mt: 2 }}>
...
const ServerNameContent = ({ settings }) => {
  ...
  return (
    <Box>
      ...
      <Stack gap={1}>
        <Typography>
          If your game server is hosted on GTX (maybe on others [to be confirmed], as long as they allow SFTP access to
          your files), you can change the server's name without restarting it. You'll first have to set : the SFTP
          credentials in your .env file (refer to the installation procedure) ; the game server IP and SFTP port in the
          GTX Server Name Change settings. Note the new name will only be visible after a map change.
        </Typography>
        <Divider flexItem />
        <TextField
          label='Server Name'
    ...
    <Box sx={{ maxWidth: (theme) => theme.breakpoints.values.md }}>
      {Object.keys(data).map((key, index) => (
        <Accordion key={key} expanded={expanded === `panel${index}`} onChange={handleChange(`panel${index}`)}>
          ...
          >
            <Typography sx={{ width: '33%', flexShrink: 0 }}>{PANEL_TITLES[key]}</Typography>
          </AccordionSummary>
          <AccordionDetails>
            <Suspense fallback={<div>Loading settings...</div>}>
              <Await resolve={data[key]} errorElement={<AsyncClientError title={PANEL_TITLES[key]} />}>
                {(resolvedSettings) => {
      ...
      <Accordion key='actions' expanded={expanded === 'actions'} onChange={handleChange('actions')}>
        <AccordionSummary expandIcon={<ExpandMoreIcon />} aria-controls={`actions-bh-content`} id={`actions-bh-header`}>
          <Typography sx={{ width: '33%', flexShrink: 0 }}>CRCON Actions</Typography>
        </AccordionSummary>
        <AccordionDetails>
          <Alert severity='warning'>Do not use this until instructed to do so by the CRCON maintainers.</Alert>
          <Typography variant='h6'>Clear Application Cache</Typography>
          <Typography>
            Clicking on this will flush the internal cache used by different CRCON components. Avoid using it
            unnecessarily, as it forces a lot of data to be recalculated and reloaded.
          </Typography>
          <Button
          ...
          <Button
            ...
            onClick={async () => {
              ...
              try {
                ...
                toast.error(error.message ?? 'Failed to reconnect to gameserver')
              } finally {
