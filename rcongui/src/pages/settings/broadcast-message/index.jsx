...
const BroadcastMessagePage = () => {
  ...
  const hasChanges = useMemo(
    () =>
      ...
      config.messages.some((message, index) => !isEqual(message, data.config.messages[index])),
    [config, data.config]
  )
  ...
  return (
    <Box sx={{ maxWidth: (theme) => theme.breakpoints.values.md }}>
      <Stack component={Paper} sx={{ p: 1, mb: 1 }} direction={'row'} justifyContent={'space-between'} gap={1}>
        <Stack direction={'row'} alignItems={'center'} gap={1}>
          <Padlock
            label={config.enabled ? 'Enabled' : 'Disabled'}
            checked={config.enabled}
            handleChange={(checked) => setConfig((prev) => ({ ...prev, enabled: checked }))}
          />
          <Padlock
            label={config.randomize ? 'Random ON' : 'Random OFF'}
            checked={config.randomize}
            handleChange={(checked) => setConfig((prev) => ({ ...prev, randomize: checked }))}
          />
        </Stack>
        <Stack direction={'row'} alignItems={'center'} gap={1}>
          <Button
      ...
      <BroadcastFields messages={config.messages} onChange={handleMessagesChange} sx={{ mb: 2 }} />
