...
const updateActionSelection = (action) => (prev) => {
  ...
  const actions = isToggled ? prev.actions.filter((aAction) => aAction !== action) : prev.actions.concat(action)
  return {
    ...prev,
    actions
  }
...
export default function LogsTable({ table, logsViewData, searchParams, setSearchParams }) {
  const [tableConfigDrawerOpen, setTableConfigDrawerOpen] = useState(false)

  const [tableConfig, setTableConfig] = useStorageState(storageKeys.LIVE_LOGS_TABLE_CONFIG, {
    density: 'normal',
    fontSize: 'normal',
    rowsPerPage: '100',
    highlighted: false,
  ...
  return (
    <>
      <TableAddons>
        <LogActionSelectionMenu actionOptions={logActionOptions} onActionSelect={handleClientActionFilterChange} />
        <LogPlayerSelectionMenu actionOptions={playerOptions} onActionSelect={handleClientPlayerFilterChange} />
      ...
      <TableConfigDrawer
        ...
        config={tableConfig}
      >
        <Typography sx={{ fontWeight: 500 }}>{'Persistent Logs Query Params'}</Typography>
        <Divider flexItem />
        <div>
          <Typography
            sx={{
            ...
            {searchParams.actions.map((action, i) => (
              <Fragment key={action}>
                ...
                <ListItem
                  disableGutters={true}
                  secondaryAction={
                    <IconButton edge='end' aria-label='delete' onClick={() => handleQueryActionParamSelect(action)}>
                      <DeleteIcon />
                    </IconButton>
                  }
                >
                  <ListItemText primary={`${_logActions[action]} - ${action}`} />
                </ListItem>
              </Fragment>
            ))}
