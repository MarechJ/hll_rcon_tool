...
export const TeamSelectionToolbar = ({ table, teamData }) => {
  const handleTeamSelect = (selectedTeam) => {
    ...
    table.setRowSelection((prev) => {
      ...
      const hasSelectedTeamMembers = allRows.some((row) => row.original.team === selectedTeam && prev[row.id])

      return allRows.reduce((acc, row) => {
        acc[row.id] = row.original.team === selectedTeam ? !hasSelectedTeamMembers : prev[row.id]
        return acc
      }, {})
    })
  }
  ...
  const handleUnitSelect = (selectedUnit) => {
    ...
    table.setRowSelection((prev) => {
      const allRows = table.getRowModel().rows
      const hasSomeNotSelected = players.some((player) => !prev[player.player_id])
  ...
  const handleCountrySelect = (selectedCountry) => {
    ...
    const allRows = table.getRowModel().rows
    const countryRows = allRows.filter((row) => row.original.country === country)
    table.setRowSelection((prev) => {
      const hasSomeNotSelected = countryRows.some((row) => !prev[row.id])

      return allRows.reduce((acc, row) => {
        acc[row.id] = row.original.country === country ? hasSomeNotSelected : prev[row.id]
        return acc
      }, {})
    })
  }
  ...
  const handleRoleSelect = (selectedRole) => {
    ...
    table.setRowSelection((prev) => {
      ...
      return allRows.reduce((acc, row) => {
        acc[row.id] = row.original.role === role ? hasSomeNotSelected : prev[row.id]
        return acc
      }, {})
    })
  }
    ...
    for (const team in teamData) {
      for (const unit_name in teamData[team].squads) {
        ...
        units.push({
          ...
          leader:
            unit.players.find((player) => ['officer', 'tankcommander', 'spotter'].includes(player.role))?.name ?? ''
        })
      }
    }
    units.sort((a, b) => `${a.team}-${a.unit_name}`.localeCompare(`${b.team}-${b.unit_name}`))
    return units
  }, [teamData])
    ...
    <TableAddons>
      <Tooltip title='Select Axis'>
        <Button onClick={() => handleTeamSelect('axis')}>
          <img src='/icons/teams/ger.webp' width={16} height={16} style={{ marginRight: 1 }} />
        </Button>
      </Tooltip>
      <Tooltip title='Select Allies'>
        <Button onClick={() => handleTeamSelect('allies')}>
          <img src='/icons/teams/us.webp' width={16} height={16} style={{ marginRight: 1 }} />
        </Button>
      </Tooltip>
      <UnitSelectionMenu unitOptions={unitOptions} onUnitSelect={handleUnitSelect} />
      <CountrySelectionMenu countryOptions={countryOptions} onCountrySelect={handleCountrySelect} />
      <RoleSelectionMenu roleOptions={roleOptions} onRoleSelect={handleRoleSelect} />
