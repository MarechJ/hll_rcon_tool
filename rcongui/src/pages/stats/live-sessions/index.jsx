...
import { getExpandedRowModel, getFilteredRowModel, getSortedRowModel, useReactTable } from '@tanstack/react-table'
...
const LiveSessionsPage = () => {
  ...
  const [tableConfig] = useStorageState(storageKeys.LIVE_PLAYERS_TABLE_CONFIG, {
    density: 'normal',
    fontSize: 'normal'
  })
  ...
  const playersData = useMemo(() => {
    ...
    const teams = players.reduce(
      (acc, player) => {
        ...
        teamName = teamName !== 'axis' && teamName !== 'allies' ? 'neutral' : teamName
    ...
    const teamsRows = Object.values(teams).map((team) =>
      Object.values(team).map((unit) => {
        ...
        if (unit.teamName !== 'neutral') {
          unitData =
            unit.unitName !== 'commmand' ? data[unit.teamName]['squads'][unit.unitName] : { type: 'armycommander' }
        }
