...
const renderSubComponent = ({ row }) => {
  ...
  const statCategories = {
    combat: [
      ...
      {
        label: 'K/D',
        value: (row.original.kills / Math.max(row.original.deaths, 1)).toFixed(2)
      },
  ...
  const players = Object.entries(row.original.most_killed || {}).map(([player, kills]) => (
    <Box key={player} sx={{ display: 'flex', justifyContent: 'space-between', py: 0.5 }}>
  ...
  const weapons = Object.entries(row.original.weapons || {}).map(([weapon, kills]) => (
    <Box key={weapon} sx={{ display: 'flex', justifyContent: 'space-between', py: 0.5 }}>
  ...
  const deathsByWeapon = Object.entries(row.original.death_by_weapons || {}).map(([weapon, deaths]) => (
    <Box key={weapon} sx={{ display: 'flex', justifyContent: 'space-between', py: 0.5 }}>
      <Typography variant='body2'>{weapon}:</Typography>
      <Typography variant='body2' sx={{ ml: 2 }}>
        {deaths}
  ...
  const deathsByPlayer = Object.entries(row.original.death_by || {}).map(([player, deaths]) => (
    <Box key={player} sx={{ display: 'flex', justifyContent: 'space-between', py: 0.5 }}>
      <Typography variant='body2'>{player}:</Typography>
      <Typography variant='body2' sx={{ ml: 2 }}>
        {deaths}
  ...
  return (
    <Box sx={{ p: 2, maxWidth: { xs: (theme) => theme.breakpoints.values.sm, lg: '100%' } }}>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(4, 1fr)' }, gap: { xs: 2, md: 4 } }}>
        <Box>
          <Typography variant='subtitle2' sx={{ mb: 1 }}>
            Kills by Player
          </Typography>
...
const PlayersTable = ({ table, selectedPlayers }) => {
  ...
  const [tableConfig, setTableConfig] = useStorageState(storageKeys.PLAYERS_TABLE_CONFIG, {
    density: 'normal',
    fontSize: 'normal'
  })
  ...
  return (
    <div>
      <TableToolbar>
        ...
        <IconButton size='small' sx={{ p: 0.5, borderRadius: 0 }} onClick={() => downloadGame(table.options.data)}>
          <DownloadIcon sx={{ fontSize: 16 }} />
        </IconButton>
        <IconButton size='small' sx={{ p: 0.5, borderRadius: 0 }} onClick={handleTableConfigClick}>
          <SettingsIcon sx={{ fontSize: 16 }} />
        </IconButton>
      </TableToolbar>
      <Table table={table} config={tableConfig} renderSubComponent={renderSubComponent} />
      <TableConfigDrawer
        name={'Players'}
