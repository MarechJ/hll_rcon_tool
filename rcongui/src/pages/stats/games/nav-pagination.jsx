...
const PaginationItem = styled((props) => <Button size='small' component={Link} {...props} />)({
  width: 20,
  height: 20,
  borderRadius: 0
})
...
export default function NavPagination({ page, maxPages, ...props }) {
  ...
  return (
    ...
    >
      ...
      <PaginationItem variant={page === 1 ? 'contained' : 'outlined'} to={`?page=${1}`}>
        1
      </PaginationItem>
      {page === 1 && 2 < maxPages && (
        <PaginationItem variant={page === 2 ? 'contained' : 'outlined'} to={`?page=${2}`}>
          2
        </PaginationItem>
      )}
      {page > 1 && page < maxPages && (
        <PaginationItem variant={page === page ? 'contained' : 'outlined'} to={`?page=${page}`}>
          {page}
        </PaginationItem>
      )}
          ...
          {!insertCustom ? (
            ...
            <Stack direction={'row'} gap={0.5} alignItems={'center'}>
              <Input
                ...
                sx={{ width: 50 }}
              />
              <Button variant={'outlined'} size={'small'} sx={{ color: green[800], '&:hover': { color: green[600] } }}>
      ...
      <PaginationItem variant={page === maxPages ? 'contained' : 'outlined'} to={`?page=${maxPages}`}>
        {maxPages}
      </PaginationItem>
