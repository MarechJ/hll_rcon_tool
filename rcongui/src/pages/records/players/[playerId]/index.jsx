import { get, execute } from '@/utils/fetchUtils'
...
export const loader = async ({ params }) => {
  ...
  const fetchPlayer = fetchResource(`get_player_profile?player_id=${playerId}`, 'Player not found')

  const fetchMessages = fetchResource(`get_player_messages?player_id=${playerId}`, 'Messages not found')

  const fetchConnectionInfo = fetchResource('get_connection_info', 'Connection info not found')

  // Run all promises concurrently
  const [profile, messages, connectionInfo] = await Promise.all([fetchPlayer, fetchMessages, fetchConnectionInfo])
...
export const action = async ({ request }) => {
  ...
  try {
    const res = await execute('post_player_comment', {
...
const PlayerInfo = () => {
  ...
  const hasVip = vips.some((vip) => vip.server_number === connectionInfo?.server_number)
  ...
  return (
    <Stack spacing={2}>
      <Stack direction={{ xs: 'column', md: 'row' }} spacing={{ xs: 1, sm: 2, md: 4 }} sx={{ width: '100%' }}>
        <Box>
          <Avatar
            style={{
              height: '150px',
          ...
          <Typography variant='h6'>
            <Link href={makePlayerProfileUrl(player_id, names[0]?.name)}>{getLinkLabel(player_id)} Profile</Link>
          </Typography>
          <Typography variant='h6'>Last connection</Typography>
          <Typography>{dayjs(sessions[0]?.end || sessions[0]?.start).format('ddd Do MMM HH:mm:ss')}</Typography>
          <Typography variant='h6'>Total play time</Typography>
          <Typography>{dayjs.duration(total_playtime_seconds, 'seconds').humanize()}</Typography>
          ...
          <Typography variant='h6'>
            <Link href={`${process.env.REACT_APP_API_URL}get_player_profile?player_id=${player_id}`}>Raw profile</Link>
          </Typography>
        </Box>
        <Box sx={{ flexGrow: 1 }}>
