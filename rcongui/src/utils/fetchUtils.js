...
export const cmd = {
  ...
  GET_CAMERA_NOTIFICATION_CONFIG: (params) =>
    requestFactory({ method: 'GET', cmd: 'get_camera_notification_config', ...params }),
  ...
  GET_VOTEKICK_AUTOTOGGLE_CONFIG: (params) =>
    requestFactory({ method: 'GET', cmd: 'get_votekick_autotoggle_config', ...params }),
  ...
  RESET_VOTEKICK_THRESHOLDS: (params) =>
    requestFactory({ method: 'POST', cmd: 'reset_votekick_thresholds', ...params }),
  ...
  SET_AUTOBALANCE_THRESHOLD: (params) =>
    requestFactory({ method: 'POST', cmd: 'set_autobalance_threshold', ...params }),
  ...
  SET_CAMERA_NOTIFICATION_CONFIG: (params) =>
    requestFactory({ method: 'POST', cmd: 'set_camera_notification_config', ...params }),
  ...
  SET_VOTEKICK_AUTOTOGGLE_CONFIG: (params) =>
    requestFactory({ method: 'POST', cmd: 'set_votekick_autotoggle_config', ...params }),
...
async function handle_response_status(response) {
  ...
  if (response.status === 504) {
    throw new CRCONServerDownError(response.statusText + '. Your server is not responding.')
  }
...
async function handle_http_errors(error) {
  ...
  if (error.name === 'AuthError') {
    ...
    toast.warn(<img src='https://media.giphy.com/media/wSSooF0fJM97W/giphy.gif' alt='bad login' />, {
      toastId: 'Bad login'
    })
  } else if (error.name === 'TypeError' || error.name === 'SyntaxError') {
    toast.error('Your RCON Api is not reachable. Check your config and start it again', {
      toastId: 'Api down'
    })
  } else {
...
async function addPlayerToWatchList(player_id, reason, playerName) {
  return postData(`${process.env.REACT_APP_API_URL}watch_player`, {
    ...
    .then((response) => showResponse(response, `Player ID ${player_id} watched`, true))
    .catch(handle_http_errors)
}

async function addPlayerToBlacklist({ blacklistId, playerId, expiresAt, reason }) {
  try {
