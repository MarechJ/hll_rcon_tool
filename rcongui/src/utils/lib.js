...
export const getVipExpirationStatus = (vip) => {
  if (!vip) return 'none'
  if (vip.vip_expiration === NEVER_EXPIRES_VIP_DATE || vip.vip_expiration === null) return 'never'
...
export const mapIdsToLayers = (layers, ids) => {
  ...
  return ids.map((id) => layers.find((layer) => layer.id === id)).filter((layer) => layer !== undefined)
}
