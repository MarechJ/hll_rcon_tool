 ...
 */
export function ActionMenu({ handleActionClick, actionList, orientation = 'vertical', ...props }) {
  ...
  const filteredActionList = useMemo(() => actionList.filter(hasPermission(user)), [actionList, user])
      ...
      <Menu id='long-menu' anchorEl={anchorEl} open={open} onClose={handleClose}>
        {filteredActionList.map((action) => (
          ...
          >
            ...
            <Typography variant='inherit' sx={{ textDecoration: action.depraceted ? 'line-through' : '' }}>
              {action.name[0].toUpperCase() + action.name.slice(1)}
            </Typography>
          </MenuItem>
        ))}
        {filteredActionList.length === 0 && <MenuItem onClick={handleClose}>No actions available</MenuItem>}
      </Menu>
    </Box>
  )
}
...
export function ActionMenuButton({
  ...
  const filteredActionList = useMemo(() => actions.filter(hasPermission(user)), [actions, user])
  ...
  return (
    <Box>
      ...
      >
        ...
        {filteredActionList.map((action) => (
          <MenuItem key={action.name} onClick={() => handleActionClick(action)} dense>
            <ListItemIcon>{action.icon}</ListItemIcon>
            <Typography variant='inherit' sx={{ textDecoration: action.depraceted ? 'line-through' : '' }}>
              {action.name[0].toUpperCase() + action.name.slice(1)}
            </Typography>
...
function hasPermission(user) {
  return (action) => {
    if (!user.is_superuser && action.permission) {
      ...
      return action.permission.every((perm) => user.permissions.some((userPerm) => userPerm.permission === perm))
    }
