...
PERMISSION_RENAMES = (
    (
        ...
        "can_view_scoreboard_config",
        "Can view scoreboard config",
    ),
    (
        "can_change_scorebot_config",
        "can_change_scoreboard_config",
        "Can change scoreboard config",
    ),
)


def rename_scoreboard_permissions(apps, schema_editor):
    Permission = apps.get_model("auth", "Permission")
    ContentType = apps.get_model("contenttypes", "ContentType")
    RconUser = apps.get_model("api", "RconUser")
    content_type = ContentType.objects.get_for_model(RconUser)

    for old_codename, new_codename, name in PERMISSION_RENAMES:
...
def restore_scorebot_permissions(apps, schema_editor):
    Permission = apps.get_model("auth", "Permission")
    ContentType = apps.get_model("contenttypes", "ContentType")
    RconUser = apps.get_model("api", "RconUser")
    content_type = ContentType.objects.get_for_model(RconUser)

    for old_codename, new_codename, _name in PERMISSION_RENAMES:
        Permission.objects.filter(
            content_type=content_type,
            codename=new_codename,
        ).update(
            codename=old_codename,
            name=old_codename.replace("_", " ").capitalize(),
        )
            ...
            options={
                ...
                "permissions": (
                    ...
                    (
                        ...
                        "Can add new shared message templates",
                    ),
                    (
                        "can_delete_message_templates",
                        "Can delete shared message templates",
                    ),
                    ...
                    (
                        ...
                        "Can view the Watch KillRate config",
                    ),
                    (
                    ...
                    (
                        "can_add_map_to_votemap",
                        "Can add map to current votemap selection",
                    ),
                    (
                        "can_set_votemap_winner",
                        "Can guarantee next map with votemap enabled",
                    ),
