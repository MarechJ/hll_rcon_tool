...
def upgrade():
    op.create_table('blacklist',
        ...
        sa.Column('sync', sa.Enum('KICK_ONLY', 'BAN_ON_CONNECT', 'BAN_IMMEDIATELY', name='blacklistsyncmethod'), nullable=False),
        sa.Column('servers', sa.Integer(), nullable=True),
        sa.PrimaryKeyConstraint('id')
    )
    op.create_table('blacklist_record',
        sa.Column('id', sa.Integer(), nullable=False),
        ...
        sa.ForeignKeyConstraint(['player_id_id'], ['player_id.id'], ),
        sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_blacklist_record_blacklist_id'), 'blacklist_record', ['blacklist_id'], unique=False)
    op.create_index(op.f('ix_blacklist_record_player_id_id'), 'blacklist_record', ['player_id_id'], unique=False)

    # Create default blacklist
    # Sequence starts at 1 so we use 0 as manual ID
    op.execute("""INSERT INTO blacklist(id, name, sync, servers)
...
def downgrade():
    ...
    # )
    op.create_index('ix_player_blacklist_player_id_id', 'player_blacklist', ['player_id_id'], unique=True)
