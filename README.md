...
__From here all the commands assume that you are at the root of the git repository you just cloned__
...
**Note**: If you make *any* changes to your `.env` you will have to recreate (`docker-compose up -d --force-recreate --remove-orphans` your containers before it will take effect), (if you `docker-compose restart` it will not take effect)
...
Note: when you're configuring multiple game servers inside one CRCON installation you need to make sure your `RCONWEB_PORT`s are unique and also not otherwise used on the machine your're hosting on, the defaults *should* work.
