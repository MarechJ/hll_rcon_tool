...
def ensure_player_id(sess: Session, player_id: str):
    if (
        sess.query(PlayerID).where(PlayerID.player_id == player_id).one_or_none()
        is None
    ):
...
class TestLogRecorder:
    def test_records_recent_logs(self, log_recorder):
        ...
        with enter_session() as sess:
            ...
            first_player = (
                sess.query(PlayerID).where(PlayerID.player_id == first_player_id).one()
            )
            assert len(res) == 1
