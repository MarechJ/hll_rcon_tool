...
class ReplaceParamsTest(TestCase):
    ...
    def test_list(self):
        assert replace_params(self.ctx, self.args, ["parameter 1", "parameter 2"]) == [
            "parameter 1",
            "parameter 2",
        ]

    def test_list_with_argument(self):
        assert replace_params(self.ctx, self.args, ["$1", "$2"]) == [
            "parameter1",
            "parameter2",
        ]

    def test_list_with_nested_argument(self):
        assert replace_params(
            self.ctx, self.args, ["$1", ["$2", "$3"], ["{player_name}"]]
        ) == ["parameter1", ["parameter2", "$3"], ["SOME_PLAYER_NAME"]]

    def test_dict(self):
        assert replace_params(
            self.ctx, self.args, {"$3 ignored": "parameter1", "key": "parameter 2"}
        ) == {"$3 ignored": "parameter1", "key": "parameter 2"}

    def test_dict_with_argument(self):
        assert replace_params(
            self.ctx, self.args, {"$3 ignored": "$2", "key": "$1"}
        ) == {"$3 ignored": "parameter2", "key": "parameter1"}

    def test_dict_with_nested_argument(self):
        assert replace_params(
            self.ctx,
            self.args,
            {"$4 ignored": "$2", "key": {"key2": "$3", "key3": "{player_name}"}},
        ) == {
            "$4 ignored": "parameter2",
            "key": {"key2": "$3", "key3": "SOME_PLAYER_NAME"},
        }

    @mock.patch(
        "rcon.rcon.Rcon.get_next_map",
        autospec=True,
        return_value=parse_layer("stmariedumont_warfare"),
    )
    def test_with_message_variables(self, _):
        assert (
            replace_params(self.ctx, self.args, "{next_map_id}")
