...
const Stream = ({ players, settings }: StreamProps) => {
  return <div className="flex items-center justify-center animate-infinite-scroll transform" style={{animationDuration: settings.animationDuration + 's'}}>
    ...
    <div className="steam-banner-text text-black text-right font-bold text-2xl pb-0.5 whitespace-pre mr-2">
      {settings.text}
    </div>
    {players.map((player =>
      <div className="h-full flex-shrink-0 px-2 py-2 text-black whitespace-nowrap flex items-center mr-3">
        <div className="steam-banner-kills text-primary text-2xl font-bold pb-0.5 mr-2">
          {player.kills}
        </div>
        ...
        <div className="steam-banner-player block">
          <div>
            {player.player}
          </div>
