...
export default function StreamBanner({ settings }: StreamBannerProps) {
  ...
  return (
    <div className="stream-banner w-full h-12 inline-flex flex-nowrap bg-white overflow-hidden">
...
const Stream = ({ players, settings }: StreamProps) => {
  return (
    <div
      className="flex items-center justify-center animate-infinite-scroll transform"
      style={{ animationDuration: settings.animationDuration + 's' }}
    >
      ...
      {players.map((player) => (
        <div
          ...
          className="h-full flex-shrink-0 px-2 py-2 text-black whitespace-nowrap flex items-center mr-3"
        >
          <div className="stream-banner-kills text-primary text-2xl font-bold pb-0.5 mr-2">{player.kills}</div>
          {settings.showAvatars && player.steaminfo?.profile?.avatar && (
            <Avatar className="size-7 mr-1">
              <AvatarImage src={player.steaminfo?.profile?.avatar} />
          ...
          <div className="stream-banner-player block">
            ...
            {settings.showWeapons && (
              <div className="stream-banner-weapon text-gray-500 text-xs">
                {Object.entries(player.weapons)
                  .sort(([n1, v1], [n2, v2]) => v2 - v1)[0]?.[0]
                  .match(/\[(.*?)\]/)?.[1] ||
                  Object.entries(player.weapons).sort(([n1, v1], [n2, v2]) => v2 - v1)[0]?.[0]}
              </div>
            )}
          </div>
        </div>
