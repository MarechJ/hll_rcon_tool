    ...
    <div className="w-full lg:w-2/3">
        ...
        <AccordionItem value="tutorial">
          <AccordionTrigger>
            <span className="w-full flex flex-row items-center justify-center">
              <GraduationCap className="inline-block size-7 mr-1" /> {t('tutorial.title')}
            </span>
          </AccordionTrigger>
          <AccordionContent>
            <StreamTutorial />
      ...
      <div className="bg-background w-full border-x border-b gap-3 p-2 flex flex-wrap">
        <div className="w-52 h-12 flex flex-col justify-between pb-2">
          ...
          <Slider
            onValueChange={(value) => setStreamSettings((prev) => ({ ...prev, playerAmount: value[0] }))}
        ...
        <div className="w-52 h-12 flex flex-col justify-between pb-2">
          ...
          <Slider
            onValueChange={(value) => setStreamSettings((prev) => ({ ...prev, animationDuration: value[0] }))}
        ...
        <div className="w-52 h-12 flex flex-col justify-between pb-2">
          ...
          <Slider
            onValueChange={(value) => setStreamSettings((prev) => ({ ...prev, pauseWidth: value[0] }))}
        ...
        <div className="w-52 text-nowrap flex-1">
          <Input
            placeholder={t('text')}
            value={streamSettings.text}
            onChange={(e) => setStreamSettings((prev) => ({ ...prev, text: e.target.value }))}
          />
        </div>
        <div className="w-52 h-10 text-nowrap items-center flex">
          <span className="pr-4">{t('showAvatars')}:</span>
          <Switch
            checked={streamSettings.showAvatars}
            onCheckedChange={(value) => setStreamSettings((prev) => ({ ...prev, showAvatars: value }))}
          />
        </div>
        <div className="w-52 h-10 text-nowrap items-center flex">
          <span className="pr-4">{t('showWeapons')}:</span>
          <Switch
            checked={streamSettings.showWeapons}
            onCheckedChange={(value) => setStreamSettings((prev) => ({ ...prev, showWeapons: value }))}
          />
        </div>
        <div className="flex-1">
          <SelectBox
            ...
            onChange={(values) =>
              Array.isArray(values) && setStreamSettings((prev) => ({ ...prev, playerFilter: values }))
            }
