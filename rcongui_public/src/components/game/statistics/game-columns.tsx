...
const teamColumn: ColumnDef<Player | PlayerWithStatus> = {
  ...
  cell: ({ row }) => {
    ...
    return (
      <TeamIndicator
        team={player.team?.confidence === 'strong' ? player.team.side : TeamEnum.MIXED}
        className="block"
      />
    )
  },
...
export const getCompletedGameColumns = (
  ...
  extras: extraColumns[] = [],
): ColumnDef<Player | PlayerWithStatus>[] => [teamColumn, playerColumn(handlePlayerClick), ...pointColumns(extras)]
