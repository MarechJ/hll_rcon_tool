...
export function RankCompareChart({ stats }: { stats: Player[] }) {
  ...
  const data = axisPlayers.map((axisPlayer, index) => ({
    rank: index + 1,
    kills: alliesPlayers[index] ? axisPlayer.kills - alliesPlayers[index].kills : null,
    axisPlayer: axisPlayer,
    alliesPlayer: alliesPlayers[index],
  }))

  const maxDiff = data.reduce(
    (prev, current) => (current.kills && Math.abs(current.kills) > prev ? Math.abs(current.kills) : prev),
    0,
  )
        ...
        >
          ...
          <Bar dataKey="kills" fill="#8884d8" isAnimationActive={false}>
            {data.map((entry, index) => (
              <Cell
                fill={
                  entry.kills && entry.kills > 0 ? getColorForTeam(TeamEnum.AXIS) : getColorForTeam(TeamEnum.ALLIES)
                }
                key={`cell-${index}`}
              />
            ))}
...
type CustomTooltipProps = {
  active?: boolean
  payload?: Array<{ payload: { axisPlayer: Player, alliesPlayer: Player, rank: number, kills: number } }>
}
  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-background border p-2 flex flex-col">
        ...
        <span>
          <TeamIndicator team={TeamEnum.AXIS} /> {payload[0].payload.axisPlayer.kills}{' '}
          {payload[0].payload.axisPlayer.player}
        </span>
        <span>
          <TeamIndicator team={TeamEnum.ALLIES} /> {payload[0].payload.alliesPlayer.kills}{' '}
          {payload[0].payload.alliesPlayer.player}
        </span>
      </div>
    )
  }
