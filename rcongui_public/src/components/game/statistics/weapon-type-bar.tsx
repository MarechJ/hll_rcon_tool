...
export const WeaponTypeBar = memo(function WeaponTypeBar({ totalKills, killsByType }: WeaponTypeBarProps) {
  const GAP_SIZE = 2.5
  ...
  const simpleWeaponTypeMap: Record<SimpleWeaponType, { color: string; order: number; t: string }> = {
    [SimpleWeaponType.Sniper]: {
      color: SIMPLE_WEAPON_TYPE_COLORS[SimpleWeaponType.Sniper],
      order: 0,
      t: t('weaponType.sniper'),
    },
    [SimpleWeaponType.Infantry]: {
      color: SIMPLE_WEAPON_TYPE_COLORS[SimpleWeaponType.Infantry],
      order: 1,
      t: t('weaponType.infantry'),
    },
    [SimpleWeaponType.MachineGun]: {
      color: SIMPLE_WEAPON_TYPE_COLORS[SimpleWeaponType.MachineGun],
      order: 2,
      t: t('weaponType.machine_gun'),
    },
    [SimpleWeaponType.Explosive]: {
      color: SIMPLE_WEAPON_TYPE_COLORS[SimpleWeaponType.Explosive],
      order: 3,
      t: t('weaponType.explosive'),
    },
    [SimpleWeaponType.Armor]: {
      color: SIMPLE_WEAPON_TYPE_COLORS[SimpleWeaponType.Armor],
      order: 4,
      t: t('weaponType.armor'),
    },
    [SimpleWeaponType.Artillery]: {
      color: SIMPLE_WEAPON_TYPE_COLORS[SimpleWeaponType.Artillery],
      order: 5,
      t: t('weaponType.artillery'),
    },
    [SimpleWeaponType.SPA]: {
      color: SIMPLE_WEAPON_TYPE_COLORS[SimpleWeaponType.SPA],
      order: 6,
      t: t('weaponType.self_propelled_artillery'),
    },
    [SimpleWeaponType.Commander]: {
      color: SIMPLE_WEAPON_TYPE_COLORS[SimpleWeaponType.Commander],
      order: 7,
      t: t('weaponType.commander'),
    },
  }
  ...
  const displayedBars = Array.from(killsBySimpleStatus.entries())
     ...
     */
    .sort(([a], [b]) => simpleWeaponTypeMap[a].order - simpleWeaponTypeMap[b].order)
      ...
      <Tooltip>
        ...
        <TooltipContent>
          {Object.entries(simpleWeaponTypeMap).map(([type, props]) => (
            <div className="flex items-center" key={props.t}>
              ...
              <span className="ml-auto">
                {killsBySimpleStatus.has(type as SimpleWeaponType) &&
                  ` (${killsBySimpleStatus.get(type as SimpleWeaponType)})`}
              </span>
            </div>
          ))}
          <div className="flex items-center">
            ...
            <span className="ml-auto">
              {totalCategorizedKills < totalKills && ` (${totalKills - totalCategorizedKills})`}
            </span>
          </div>
        </TooltipContent>
      </Tooltip>
